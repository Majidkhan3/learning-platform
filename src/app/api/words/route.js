...
export async function POST(req) {
  const auth = await verifyToken(req)
  if (!auth.valid) {
    console.log('[AUTH FAIL]', auth.error)
    return NextResponse.json({ error: auth.error || 'Unauthorized' }, { status: 401 })
  }
  ...
  try {
    ...
    if (!word || !userId) {
      console.log('[VALIDATION FAIL] word or userId missing')
      return NextResponse.json({ error: "The 'word' and 'userId' parameters are required." }, { status: 400 })
    }
    ...
    if (autoGenerateSummary) {
      ...
      if (!promptTemplate) {
        promptTemplate = `Generate a detailed synthesis for the word {{word}}...`
      }
      ...
      const claudeApiKey = process.env.CLAUDE_API_KEY
      if (!claudeApiKey) {
        console.log('[CLAUDE ERROR] Missing API key')
      }
      ...
      if (claudeResponse.ok) {
        ...
        generatedSummary =
          ...
          claudeResult?.completion?.trim() ||
          generatedSummary
        console.log('[CLAUDE RESPONSE]', generatedSummary.slice(0, 100))
      } else {
        console.log('[CLAUDE FAILED]', await claudeResponse.text())
      }
    }

    if (autoGenerateImage) {
      console.log('[IMAGE GEN] Requesting image generation for:', word)

      const openAiApiKey = process.env.OPENAI_API_KEY
      if (!openAiApiKey) {
        console.log('[OPENAI ERROR] Missing API key')
      ...
      try {
        ...
        if (openAiResponse.ok) {
          ...
          try {
            ...
            updatedImage = cloudinaryResult.secure_url
            console.log('[IMAGE UPLOADED]', updatedImage)
          } catch (err) {
            console.log('[CLOUDINARY FALLBACK]', err)
            updatedImage = generatedImageUrl
          }
        } else {
          console.log('[OPENAI ERROR]', await openAiResponse.text())
        }
      } catch (err) {
    ...
    await newWord.save()
    console.log('[DB] Word saved:', newWord)
...
export async function GET(req) {
  const auth = await verifyToken(req)
  if (!auth.valid) {
    console.log('[AUTH FAIL - GET]', auth.error)
    return NextResponse.json({ error: auth.error || 'Unauthorized' }, { status: 401 })
  }
  ...
  try {
    ...
    const words = await Word.find({ userId })
    console.log('[GET WORDS]', words.length)
