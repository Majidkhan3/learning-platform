...
import Gerstories from '../../../../model/Gerstories'
...
export async function GET(req) {
  ...
  try {
    ...
    const stories = await Gerstories.find({ userId })
...
async function generateStoryWithClaude(words, theme) {
  try {
    ...
    const prompt = `
      Please create exactly 2 natural and coherent narrative dialogues in German, simulating a real conversation between two people.
...
function generateStoryTitlePrompt(storyText) {
  return `
Based on the following story (which contains exactly two dialogues in German), generate a short title of 3 to 4 words maximum that summarizes the main topic of the content.

Stories:
${storyText.substring(0, 800)}...

Please respond with only the title, without quotation marks or periods. The title must be in German and capture the narrative essence of the story.
`
}
