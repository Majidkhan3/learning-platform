...
import Gertags from '../../../../model/Gertags'
...
export async function POST(req) {
  ...
  try {
    ...
    const existingTag = await Gertags.findOne({ name, userId })
    if (existingTag) {
      return NextResponse.json({ error: 'Tag already exists' }, { status: 400 })
    }

    // Create a new tag
    const newTag = new Gertags({ name, userId })
    await newTag.save()
...
export async function GET(req) {
  ...
  try {
    ...
    const tags = await Gertags.find({ userId })
...
export async function DELETE(req) {
  ...
  try {
    ...
    // Delete the tag
    const deletedTag = await Gertags.findOneAndDelete({ name, userId })
        await Gerword.updateMany(
      { userId, tags: name },
      { $pull: { tags: name } }
    )
