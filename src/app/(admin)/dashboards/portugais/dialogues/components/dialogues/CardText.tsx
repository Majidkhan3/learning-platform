...
const CardText = () => {
  ...
  return (
    <>
      ...
      <div className="mb-4">
        <Link href="/dashboards/portugais/dialogues/youtube">
          <Button variant="primary">Use um vídeo do YouTube</Button>
        </Link>
      </div>

      <Card className="mb-4">
        <Card.Header className="bg-light fw-bold">Enviar um arquivo PDF</Card.Header>
        <Card.Body>
          <Form onSubmit={handleSubmit}>
            <Form.Group controlId="formFile" className="mb-3">
              <Form.Label>Arquivo PDF (podcast em portugase)</Form.Label>
              <Form.Control
                id="file-upload"
                type="file"
              ...
              />
              <small className="text-muted">Diálogos serão gerados usando Claude (Anthropic)</small>
            </Form.Group>
            {isProcessing && (
      ...
      <h5 className="fw-bold mb-3">Arquivos processados</h5>
      ...
      .map((dialogue: DialogueEntry) => (
        <Card className="mb-3" key={dialogue._id}>
          <Card.Body>
            <p className="mb-1">
              <strong>
                {/* show title if you stored one, otherwise URL */}
            ...
            <div className="d-flex gap-2">
              <Link
                href={`/dashboards/portugais/dialogues/view/${dialogue._id}`}
              >
