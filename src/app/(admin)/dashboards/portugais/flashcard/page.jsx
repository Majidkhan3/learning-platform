...
const FlashCard = () => {
  ...
  useEffect(() => {
    const speakWithPolly = async (text) => {
      try {
        const response = await fetch('/api/polly?language=pt-BR', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({
            text,
            voice: 'Camila', // Use a specific voice for Portuguese
            language: 'pt-BR', // Adjust language as needed
          }),
        })
...
const handleKeyPress = useCallback(
  (e) => {
    ...
    switch (e.key) {
      ...
      case 'ArrowLeft':
        ...
        break;
      case 'Escape':  // ✅ NEW
        router.push('/dashboards/portugais'); // Or wherever the word list is
        break;
      default:
        break;
    }
  ...
  <Button
    variant="link"
    className="me-3 p-0"
    onClick={() => router.push('/dashboards/portugais')}
  >
    <IconifyIcon icon="bi:arrow-left" width={20} />
  </Button>
  <h4 className="mb-0">Cartão de memória</h4>
</div>
    <div
