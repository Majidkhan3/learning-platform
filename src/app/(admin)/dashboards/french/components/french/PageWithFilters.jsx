...
const PageWithFilters = () => {
  ...
  const fetchTags = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/french/frtags?userId=${userId}`); // Replace with your API endpoint
  ...
  const fetchWords = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/french/frword?userId=${userId}`); // Replace with your API endpoint
      const data = await res.json();
      if (data.success) {
