...
export default function AddWordsPage() {
  ...
  const fetchWords = async () => {
    try {
      setLoading(true)
      const res = await fetch(`/api/german/gerword?userId=${userId}`, {
        headers: {
  ...
  useEffect(() => {
    const fetchTags = async () => {
      try {
        ...
        const response = await fetch(`/api/german/gertags?userId=${user._id}`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          },
...
const handleSubmit = async () => {
  ...
  try {
    ...
    for (let i = 0; i < filteredWords.length; i++) {
      ...
      const response = await fetch(`/api/german/gerword`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
    ...
    <Row>
      <Col lg={12}>
        <Container className="mt-4" fluid>
          <h2>➕ Mehrere Wörter hinzufügen</h2>
          <Card className="mt-3">
            <Card.Body>
              <h5>Formular zum Hinzufügen mehrerer Wörter</h5>
              <p className="text-muted">Mehrere deutsche Wörter gleichzeitig hinzufügenz</p>
              ...
              <Row>
                <Col md={8}>
                  <Form.Group controlId="wordList">
                    <Form.Label>Geben Sie pro Zeile ein Wort ein:</Form.Label>
                    <Form.Control
                      as="textarea"
                      rows={8}
                      placeholder="Beispiel: Haus&#10;Hund&#10;Katze&#10;Buch"
                      value={words}
                ...
                <Col md={4}>
                  <Form.Group controlId="tags">
                    <Form.Label>Schlagwörter (einen oder mehrere auswählen):</Form.Label>
                    {fetchingTags ? (
                    ...
                    <Form.Text muted>Halten Sie Strg (oder Cmd) gedrückt, um mehrere Tags auszuwählen.</Form.Text>

                    <div className="mt-3">
                      <Form.Check
                        type="checkbox"
                        label="Bild automatisch generieren"
                        checked={autoGenerateImage}
                        onChange={(e) => setAutoGenerateImage(e.target.checked)}
                      />
                      <Form.Check
                        type="checkbox"
                        label="Zusammenfassung automatisch generieren"
                        checked={autoGenerateSummary}
                        onChange={(e) => setAutoGenerateSummary(e.target.checked)}
                      />
                      <Form.Check
                        type="checkbox"
                        label="Vorhandene Wörter automatisch ignorieren"
              ...
              <Row className="mt-4">
                <Col>
                  <h5>Sternebewertungen für die Wörter festlegen</h5>
                  {words
                    .split('\n')
                    .map((word) => word.trim())
                    .filter((word) => word !== '')
              ...
              <div className="mt-4 d-flex justify-content-between">
                ...
                <Button variant="success" onClick={handleSubmit} disabled={loading}>
                  {loading ? 'Senden...' : '✅ Wörter hinzufügen'}
                </Button>
              </div>
            </Card.Body>
          </Card>
