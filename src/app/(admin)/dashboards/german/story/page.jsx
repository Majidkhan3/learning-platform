...
const Page = () => {
  ...
  const handleDelete = async (storyId) => {
    if (window.confirm('Are you sure you want to delete this story?')) {
      try {
        const response = await fetch(`/api/german/gerstories/create/${storyId}`, {
          method: 'DELETE',
          headers: {
  ...
  const handleStoryTitleSave = async (storyId) => {
    ...
    setSavingTitle(true)
    try {
      const res = await fetch(`/api/german/gerstories/create/${storyId}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  useEffect(() => {
    const fetchStories = async () => {
      ...
      try {
        setLoading(true)
        const res = await fetch(`/api/german/gerstories/create?userId=${userId}`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <Container className="py-5">
      ...
      <Row className="align-items-center mb-4">
        <Col>
          <h2>
            <strong>DE</strong> Geschichten auf Deutsch
          </h2>
        </Col>
        <Col className="text-end">
          <Stack direction="horizontal" gap={2} className="justify-content-end">
            <Button variant="outline-primary">Wortschatz</Button>
            <Button variant="outline-primary">Dialoge</Button>
          </Stack>
        </Col>
      </Row>
      ...
      <Row className="align-items-center mb-3">
        <Col>
          <h4>Alle Geschichten</h4>
        </Col>
        <Col className="text-end">
          <Button variant="success" onClick={() => router.push('/dashboards/german/story/create')}>
            <Icon icon="ic:round-plus" className="me-2" />
            Erstellen Sie eine neue Geschichte
          </Button>
        </Col>
      </Row>
      <Row>
      ...
      <Row xs={1} sm={2} md={3} className="g-4">
        {loading ? (
          <Col className="text-center w-full justify-center py-5">
            ...
            <p className="text-muted">Geschichten werden geladen...</p>
          </Col>
        ) : (
          <>
            {[...stories]
              ...
              .map((story) => (
                <Col key={story.storyId}>
                  <Card>
                    <Card.Header className="d-flex justify-content-between align-items-center">
                      {editingStoryId === story.storyId ? (
                        <div className="d-flex align-items-center gap-2 w-100">
                          ...
                          >
                            {savingTitle ? <Icon icon="eos-icons:loading" width={16} height={16} /> : 'Speichern'}
                          </Button>
                        ...
                        <strong
                          role="button"
                          title="Doppelklick zum Bearbeiten des Titels"
                    ...
                    <Card.Body>
                      <Card.Text className="mb-2">{story.rating || 'Keine Bewertung'}</Card.Text>
                      <Card.Text className="mb-2">{story.tags?.join(', ') || 'Keine Tags'}</Card.Text>
                      ...
                      <Card.Text>
                        <strong>Thema:</strong> {story.theme}
                      </Card.Text>
                      <Button
                        variant="primary"
                        className="w-100 mt-3"
                        onClick={() => router.push(`/dashboards/german/story/view/${story.storyId}`)}
                      >
                        Geschichte ansehen
                      </Button>
                    </Card.Body>
