...
const preprocessDialogues = (dialogueString) => {
  ...
  lines.forEach((line) => {
    ...
    if (trimmedLine.includes('Person A:')) {
      currentDialogue.a = trimmedLine.split('Person A:')[1]?.trim()
    } else if (trimmedLine.includes('Person B:')) {
...
if (path.includes('/portugais')) language = 'pt-PT'
  ...
  useEffect(() => {
    const fetchStory = async () => {
      try {
        setLoading(true)
        const res = await fetch(`/api/german/gerstories/create/${id}`,{
          headers: {
...
const speakSingle = async (text, voice) => {
  ...
  try {
    ...
    const response = await fetch(`/api/polly?${language}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
          ...
          <Card className="mb-4">
            ...
            <Card.Body>
              <Row>
                <Col md={6}>
                  <Form.Group>
                    <Form.Label>Stimme für die Person A:</Form.Label>
                ...
                <Col md={6}>
                  <Form.Group>
                    <Form.Label> Stimme für die Person B:</Form.Label>
