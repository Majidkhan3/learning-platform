...
const FilterCard = ({ tags, voices, onVoiceChange }) => {
  ...
  const [selectedTags, setSelectedTags] = useState(['All']) // Change to array
  const [selectedRatings, setSelectedRatings] = useState(['All']) // Change to array
  const [voice, setVoice] = useState(voices[0]?.id || 'Lucia')
  const [flashCardMode, setFlashCardMode] = useState(false)

  const ratings = ['All','Keine Bewertung', '1', '2', '3', '4']
  ...
  const handleFlashCardModeChange = (e) => {
    ...
    if (isChecked) {
      ...
      router.push(`/dashboards/german/${flashcardPath}${query ? `?${query}` : ''}`)
    }
  }
  ...
  return (
    <Card className="mb-4">
      <Card.Body>
        <h5 className="mb-3">Nach Schlagwort filtern:</h5>
        ...
        <h5 className="mb-3">Nach Bewertung filtern</h5>
        <div className="d-flex flex-wrap gap-2 mb-4">
          {ratings.map((rating) => (
            <Badge
              key={rating}
              ...
              style={{ cursor: 'pointer' }}>
              {(rating === 'All' || rating === 'Keine Bewertung') ? rating : `${rating} ★`} 
            </Badge>
          ))}
        </div>
