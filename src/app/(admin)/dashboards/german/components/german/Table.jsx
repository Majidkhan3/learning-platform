...
const Table = ({ loading, words, selectedVoice }) => {
  ...
  useEffect(() => {
    ...
    const filtered = words.filter((word) => {
      ...
      const matchesRating =
        ...
        (selectedRatings.includes('Keine Bewertung') && (!word.note || word.note === 0)) || // <-- NEW: No rating filter
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`/api/german/gerword/${id}`, {
        method: 'DELETE',
        headers: {
          ...
          <CardFooter className="py-2">
            <nav aria-label="Page navigation">
              <ul className="pagination pagination-sm justify-content-end mb-1">
                ...
                {/* Next */}
                <li
                  className={`page-item ${currentPage === totalPages || filteredData.length === 0
                      ? "disabled"
                      : ""
                    }`}
                >
