...
const SynthesisModal = ({ reviewData, loading, onDelete, selectedVoice }) => {
  ...
  const handleRating = async (itemId, star, item) => {
    try {
      const res = await fetch(`/api/german/gerword/${itemId}`, {
        method: "PUT",
        headers: {
  ...
  const speakWord = async (word) => {
    try {
      ...
      const response = await fetch('/api/polly?language=de-DE', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleEditClick = (id) => {
    router.push(`/dashboards/german/edit/${id}`) // Navigate to the edit page with the item's ID
  }
              ...
              <tr key={idx}>
                ...
                <td>
                  <Button
                    ...
                    size="sm"
                    onClick={() => window.open(`https://youglish.com/pronounce/${item.word}/german`, '_blank')}>
