...
const AddTags = () => {
  ...
  const fetchTags = async () => {
    try {
      const res = await fetch(`/api/german/gertags?userId=${userId}`,{ headers: {
      ...
      if (data.success) {
        ...
        const wordsRes = await fetch(`/api/german/gerword?userId=${userId}`,{ headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },})
  ...
  const handleAddTag = async () => {
    ...
    try {
      const res = await fetch('/api/german/gertags', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleDeleteTag = async (tagName) => {
    try {
      const res = await fetch(`/api/german/gertags?name=${tagName}&userId=${userId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        ...
        <Card>
          <Card.Body>
            <div className="d-flex align-items-center mb-4">
              ...
              <h4 className="mb-0"> Etikettenverwaltung</h4>
            </div>

            {/* Add New Tag Section */}
            <div className="mb-4">
              <h5>Füge ein neues Tag hinzu</h5>
              <Form.Group className="mb-3">
                <Form.Label>Namen der neuen Labels</Form.Label>
                <div className="d-flex">
                  <Form.Control type="text" value={newTagName} onChange={(e) => setNewTagName(e.target.value)} placeholder=" Geben Sie den Tag-Namen ein." />
                  ...
                  <ListGroup.Item key={index} className="d-flex justify-content-between align-items-center">
                    ...
                    <div className="d-flex align-items-center">
                      <Badge bg="light" text="dark" className="me-2">
                        {tag.count} Wörter{tag.count !== 1 ? 's' : ''}
                      </Badge>
