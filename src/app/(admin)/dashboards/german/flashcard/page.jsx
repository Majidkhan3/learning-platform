...
const FlashCard = () => {
  ...
  const fetchWords = async () => {
    try {
      ...
      let apiUrl = `/api/german/gerword?userId=${userId}`
      if (tag) {
      ...
      if (data.success) {
        ...
        const filteredCards = data.words.filter((word) => {
          ...
          if (rating && rating !== 'All') {
            ...
            if (selectedRatings.includes('Keine Bewertung')) {
        ...
        if (mappedCards.length > 0 && currentIndex > mappedCards.length) {
          ...
          router.push(`/dashboards/german/flashcard?tag=${tag || ''}&rating=${rating || ''}&index=1`)
    ...
    const speakWithPolly = async (text) => {
      try {
        const response = await fetch('/api/polly', {
          ...
          body: JSON.stringify({
            text,
            voice: 'Marlene', // Use the "Marlene" voice
            language: 'de-DE', // Adjust language as needed
          }),
        })
  ...
  const goToCard = useCallback((index) => {
    if (index >= 1 && index <= cards.length) {
      if (tag && rating) {
        router.push(`/dashboards/german/flashcard?tag=${tag}&rating=${rating}&index=${index}`)
      } else if (tag) {
        router.push(`/dashboards/german/flashcard?tag=${tag}&index=${index}`)
      } else if (rating) {
        router.push(`/dashboards/german/flashcard?rating=${rating}&index=${index}`)
      } else {
        router.push(`/dashboards/german/flashcard?index=${index}`)
      }
      setIsFlipped(false)
    }
    ...
    (e) => {
      ...
      switch (e.key) {
        ...
        case 'ArrowLeft':
          ...
          break;
        case 'Escape':  // ✅ NEW
          router.push('/dashboards/german'); // Or wherever the word list is
          break;
        default:
          break;
      }
  ...
  const handleRating = async (star) => {
    ...
    try {
      ...
      const response = await fetch(`/api/german/gerword/${currentCard.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleYouGlish = (word) => {
    if (word) {
      window.open(`https://youglish.com/pronounce/${word}/german`, '_blank')
    }
  }
  ...
  if (loading)
    return (
      <Col className="text-center w-full justify-center py-5">
        ...
        <p className="text-muted">Lade Lernkarten...</p>
      </Col>
    )
    ...
    <>
      <div className="d-flex align-items-center mb-4">
        <Button
          variant="link"
          className="me-3 p-0"
          onClick={() => router.push('/dashboards/german')}
        >
          <IconifyIcon icon="bi:arrow-left" width={20} />
        </Button>
        <h4 className="mb-0">Lernkarte</h4>
      </div>
      <div
      ...
      >
        <div className="flashcard-container" style={{ width: '400px' }}>
          <div className="text-center mb-2">
            <small>
              Karte {currentIndex} of {cards.length}
            </small>
          </div>
                ...
                <Card.Body className="d-flex flex-column h-100">
                  ...
                  <div className="mt-auto">
                    <div className="text-center mb-3">
                      <small>Tippe, um den Inhalt zu zeigen</small>
                    </div>
                  </div>
                </Card.Body>
              </Card>
              ...
              <Card
                ...
                <Card.Body>
                  ...
                  <Card.Subtitle className="mb-3 text-center d-flex justify-content-center align-items-center">
                    Synthesis
                    <i className="bi bi-info-circle ms-2" title="Zusammenfassung"></i> {/* Synthesis Icon */}
                  </Card.Subtitle>
                  <div className="text-center mb-3">
                    {currentCard?.image ? (
                      <img
                  ...
                  {/* Rating Section */}
                  <div className="mb-3">
                    <strong>Califica esta tarjeta</strong>
        ...
        <Modal show={showModal} onHide={closeModal} centered>
          <Modal.Header closeButton>
            <Modal.Title>Bildvorschau</Modal.Title>
          </Modal.Header>
