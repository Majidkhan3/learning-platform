...
if (path.includes('/portugais')) language = 'pt-PT'
...
else if (path.includes('/german')) language = 'de-DE'
  useEffect(() => {
    if (id) {
      fetch(`/api/german/gerdialogues/${id}`,
        {headers: {
  ...
  const parseDialogues = (dialogueString) => {
    ...
    lines.forEach((line) => {
      if (line.includes('Person A:') || line.includes('Personne A:')) {
        currentDialogue.a = line.split(/Person A:|Personne A:/)[1]?.trim()
      } else if (line.includes('Person B:') || line.includes('Personne B:')) {
        currentDialogue.b = line.split(/Person B:|Personne B:/)[1]?.trim()
...
const speak = async (text, voiceLabel) => {
  ...
  return (
    <div>
      ...
      <Card className="mb-4">
        <Card.Header className="bg-primary text-white">Einstellungen für die Sprachsynthese (Amazon Polly)</Card.Header>
        <Card.Body>
          <Row>
            <Col md={6}>
              <Form.Group>
                <Form.Label>Stimme für Person A:</Form.Label>
            ...
            <Col md={6}>
              <Form.Group>
                <Form.Label>Stimme für Person B:</Form.Label>
            ...
            <Row>
              <Col md={6}>
                <div className="d-flex align-items-center justify-content-between">
                  ...
                  <Button variant="link" onClick={() => speak(conv.a, voiceA)} disabled={isSpeaking} title="Diesen Text vorlesen">
              ...
              <Col md={6}>
                <div className="d-flex align-items-center justify-content-between">
                  ...
                  <Button variant="link" onClick={() => speak(conv.b, voiceB)} disabled={isSpeaking} title="Diesen Text vorlesen">
                    <IconifyIcon icon="ri:volume-up-line" className="align-middle fs-18" />
                  </Button>
                </div>
                <p>{conv.b}</p>
