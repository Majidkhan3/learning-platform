...
const CardText = () => {
  ...
  const generateDialoguesFromApi = useCallback(
    async (text: string) => {
      ...
      setLoading(true)
      try {
        const res = await fetch('/api/german/gerdialogues/create', {
          method: 'POST',
          headers: {
  ...
  useEffect(() => {
    const fetchDialogues = async () => {
      ...
      try {
        setFetching(true)
        const res = await fetch(`/api/german/gerdialogues?userId=${user._id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
  ...
  const handleDelete = async (dialogueId: string, e: React.MouseEvent) => {
    ...
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce dialogue ?')) {
      try {
        const res = await fetch(`/api/gerdialogues/${dialogueId}`, {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
  ...
  return (
    <>
      ...
      <div className="mb-4">
        <Link href="/dashboards/german/dialogues/youtube">
          <Button variant="primary">YouTube-Video verwenden</Button>
        </Link>
      </div>

      <Card className="mb-4">
        <Card.Header className="bg-light fw-bold">Senden Sie eine PDF-Datei</Card.Header>
        <Card.Body>
          <Form onSubmit={handleSubmit}>
            <Form.Group controlId="formFile" className="mb-3">
              <Form.Label>PDF-Datei (Podcast auf Deutsch)</Form.Label>
              <Form.Control
                id="file-upload"
                type="file"
              ...
              />
              <small className="text-muted">Die Dialoge werden mit Claude (Anthropic) generiert.</small>
            </Form.Group>
            {isProcessing && (
            ...
            <Button variant="primary" type="submit" disabled={loading || isProcessing || !extractedText.trim()}>
              {loading ? (
                <>
                  ...
                  <span className="ms-1">	Generierung...</span>
                </>
              ) : (
                'Senden und Dialoge generieren'
              )}
            </Button>
          </Form>
        </Card.Body>
      ...
      <h5 className="fw-bold mb-3">Verarbeitete Dateien</h5>
      <div className="d-flex justify-content-end mb-3">
        <Form.Select
          ...
          <option value="newest">Von der neuesten zur ältesten</option>
          <option value="oldest">Von der ältesten zur neuesten</option>
        </Form.Select>
      </div>
      {fetching ? (
            ...
            <Card className="mb-3" key={dialogue._id}>
              <Card.Body>
                ...
                <div className="d-flex gap-2">
                  <Link href={`/dashboards/german/dialogues/view/${dialogue._id}`}>
