          ...
          try {
            if (wordData.word.summary) {
              if (wordData.word.summary.trim().startsWith('{')) {
                editorState = EditorState.createWithContent(convertFromRaw(JSON.parse(wordData.word.summary)))
              } else {
                editorState = EditorState.createWithContent(ContentState.createFromText(wordData.word.summary))
              }
            } else {
  ...
  const saveContent = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        summary: formData.autoGenerateSummary ? undefined : JSON.stringify(convertToRaw(formData.summary.getCurrentContent())),
        userId,
      }
        ...
        <Card>
          <Card.Body>
            ...
            <Form.Group className="mb-4">
              ...
              <Form.Control type="text" value={formData.word} onChange={handleWordChange} placeholder="Enter the word" />
            </Form.Group>
            ...
            <Form.Group className="mb-4">
              ...
              {!formData.autoGenerateImage && (
                <div className="mt-3">
                  ...
                  {formData.image && (
                    <div className="d-flex align-items-center gap-3">
                      <Image src={formData.image} alt="Preview" width={100} height={100} className="border rounded" style={{ objectFit: 'cover' }} />
                      <Button variant="outline-danger" size="sm" onClick={() => setFormData((prev) => ({ ...prev, image: '' }))}>
                        Remove Image
                      </Button>
                    </div>
                  )}
              ...
              {formData.autoGenerateImage && (
                <small className="text-muted d-block mt-2">An image will be automatically generated based on the word.</small>
              )}
            </Form.Group>

            {error && <div className="text-danger mb-3">{error}</div>}

            <Button variant="primary" size="lg" onClick={saveContent} disabled={loading}>
