...
const FlashCard = () => {
  ...
  const fetchWords = async () => {
    try {
      ...
      let apiUrl = `/api/english/enword?userId=${userId}`
      if (tag) {
      ...
      if (data.success) {
        ...
        const filteredCards = data.words.filter((word) => {
          ...
          if (rating && rating !== 'All') {
            ...
            if (selectedRatings.includes('No rating')) {
        ...
        if (mappedCards.length > 0 && currentIndex > mappedCards.length) {
          ...
          router.push(`/dashboards/english/flashcards?tag=${tag || ''}&rating=${rating || ''}&index=1`)
  ...
  const goToCard = useCallback((index) => {
    if (index >= 1 && index <= cards.length) {
      if (tag && rating) {
        router.push(`/dashboards/english/flashcards?tag=${tag}&rating=${rating}&index=${index}`)
      } else if (tag) {
        router.push(`/dashboards/english/flashcards?tag=${tag}&index=${index}`)
      } else if (rating) {
        router.push(`/dashboards/english/flashcards?rating=${rating}&index=${index}`)
      } else {
        router.push(`/dashboards/english/flashcards?index=${index}`)
      }
      setIsFlipped(false)
    }
    ...
    (e) => {
      ...
      switch (e.key) {
        ...
        case 'ArrowLeft':
          ...
          break;
        case 'Escape':  // ✅ NEW
          router.push('/dashboards/english'); // Or wherever the word list is
          break;
        default:
          break;
      }
    ...
    <>
      <div className="d-flex align-items-center mb-4">
        <Button
          variant="link"
          className="me-3 p-0"
          onClick={() => router.push('/dashboards/english')}
        >
